...
class SecureSecretV2 {
  ...
  cli() {
    ...
    if (params[2].valor == 0) {
      fs.writeFileSync(params[4].valor, this.encrypt(params[0].valor, params[1].valor, fs.readFileSync(params[3].valor)));
      console.log("File encrypted to: " + params[4].valor);
    }
    if (params[2].valor == 1) {
      fs.writeFileSync(params[4].valor, this.decrypt(params[0].valor, params[1].valor, fs.readFileSync(params[3].valor )));
  ...
  encrypt(algorithm, password, buffer) {
    ...
    const dataBuffer = Buffer.from(cipher.update(buffer, undefined, "hex"))

    const finalBuffer = Buffer.from(cipher.final("hex"))

    return Buffer.concat([dataBuffer, finalBuffer])
  }
  decrypt(algorithm, password, buffer) {
    ...
    const dataBuffer = decipher.update(buffer.toString(), "hex")
    console.log("dataBuffer", dataBuffer, dataBuffer.toString())
